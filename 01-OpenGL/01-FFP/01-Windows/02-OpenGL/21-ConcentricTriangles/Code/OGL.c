...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	//create window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("NKK OGL - Assignment 12/16"), WS_OVERLAPPEDWINDOW, (xPos - (WIN_WIDTH / 2)),
...
void display(void)
{
	void drawHollowTriangle(float centerX, float centerY, float radius);
	...
	for (yCounter = 0; yCounter < 2; yCounter++)
	{
		for (xCounter = 0; xCounter < 5; xCounter++)
		{
			...
			drawHollowTriangle(0.0f, 0.0f, 0.5f);
			xPos += 1.5;
