...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	//create window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("NKK OGL - Assignment 2/16"), WS_OVERLAPPEDWINDOW, (xPos - (WIN_WIDTH / 2)),
...
void display(void)
{
	...
	//Assignment 2/16
	/*Assignment 2:
	-Horizontally Centered line dividing the screen
		- Red color line and having visibly thick width
		- This is the x - axis from - ve to + ve.*/
	glPointSize(1.0f);
	glBegin(GL_POINTS);
	glColor3f(1.0f, 0.0f, 0.0f);
	float fPointCounterX = 0.0f;
	for (fPointCounterX = -5.0f; fPointCounterX <= 5.0f; fPointCounterX = fPointCounterX + 0.001f)
	{
		glVertex3f(fPointCounterX, 0.0f, 0.0f);
	}
