...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	//create window
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("NKK OGL - PP Perspective Triangle-Rectangle with Shaders 7th July"), WS_OVERLAPPEDWINDOW, (xPos - (WIN_WIDTH / 2)),
...
int initialize(void)
{
	...
	*/
	...
	glBindAttribLocation(shaderProgramObject, AMC_ATTRIBUTE_COLOR, "aColor"); /*Vulkan : Map->memcpy->unMap */
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position_triangle); //binding point (it can have multiple binding points)
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_color_triangle); //binding point (it can have multiple binding points)
	glBufferData(GL_ARRAY_BUFFER, sizeof(triangle_color), triangle_color, GL_STATIC_DRAW);
	glVertexAttribPointer(AMC_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL); //(Dhanga)interlived la 0 ha 0 asanar nahiye
	//#define NULL (void*) &0;
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position_rectangle); //binding point (it can have multiple binding points)
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_color_rectangle); //binding point (it can have multiple binding points)
	...
	/*If we wish to avoid vao and vbo of color 
	use following code
...
void display(void)
{
	...
	//drawing code
	//transformations 
	modelViewMatrix = mat4::identity(); //this is analogus to glLoadIdentity in display() for model view matrix
	modelViewProjectionMatrix = mat4::identity(); //glMatrixMode(GL_MODELVIEW); 
